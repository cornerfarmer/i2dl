...
class TwoLayerTask(TaskPlan.Task):
    ...
    def step(self, tensorboard_writer, current_iteration):
        loss, acc = self.net.step(self.X_train, self.y_train, learning_rate=self.preset.get_float('learning_rate'), reg=self.preset.get_float('reg'), batch_size=self.preset.get_int('batch_size'))
